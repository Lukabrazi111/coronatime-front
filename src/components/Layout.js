import React, {useState, useEffect} from "react";

const Layout = (props) => {
    const [open, setOpen] = useState(false);

    // useEffect(() => {
    //
    //     document.addEventListener('mousedown', () => {
    //         setOpen(false);
    //     })
    // }, [])

    return (
        <React.Fragment>
            <div className="absolute right-1/2 top-2">
                <ul>
                    <li>
                        <a onClick={() => setOpen(!open)} className="flex items-center cursor-pointer">
                            English
                            <span>
                                <img className="ml-2" src={require('../assets/img/Stroke 165.png')} alt="arrowDown"/>
                            </span>
                        </a>
                        {open && <div
                            className="shadow-md w-36 absolute left-0 top-9 rounded-lg bg-gray-200 bg-opacity-75 text-black">
                            <ul>
                                <li>
                                    <a href='#en'
                                       className="text-left px-4 p-3 transition duration-150 ease-in hover:bg-gray-300 rounded w-full block">
                                        English
                                    </a>
                                </li>

                                <li>
                                    <a href="#ka"
                                       className="text-left px-4 p-3 transition duration-150 ease-in hover:bg-gray-300 rounded w-full block">
                                        ქართული
                                    </a>
                                </li>
                            </ul>
                        </div>}
                    </li>
                </ul>
            </div>

            {props.children}
        </React.Fragment>
    );
};

export default Layout;